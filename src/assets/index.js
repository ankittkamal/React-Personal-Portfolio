export { default as Hero } from "./img/hero.png";
export { default as Leaf1 } from "./img/Leaf.png";
export { default as Leaf2 } from "./img/leaf2.png";
export { default as about } from "./img/about.webp";

// projects

export { default as SocialMedia } from "./img/Project/thumb-fullstack-socialmedia-app-react-sanity.png";
export { default as PortfolioFirebase } from "./img/Project/PortfolioWebiste.png";
export { default as NetflixGPT } from "./img/Project/NetflixGPT.png";
export { default as ReactRestaurant } from "./img/Project/ReactRestaurant.png";
export { default as MoneyTracker } from "./img/Project/MoneyTracker.png";
export { default as AnkitKamalResume } from "./Ankit_Kamal_Resume.pdf";
export { default as ReactEats } from "./img/Project/ReactEats.png";
export { default as Mapty } from "./img/Project/Mapty.png";
